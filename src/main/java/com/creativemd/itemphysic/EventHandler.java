...
public class EventHandler {
    ...
    public static EntityItem getEntityItem(double distance, EntityPlayer player) {
         ...
         * Entity entity = player.worldObj.findNearestEntityWithinAABB(EntityItem.class,
         * player.boundingBox.addCoord(vec31.xCoord * reach, vec31.yCoord * reach, vec31.zCoord *
         * reach).expand((double)f1, (double)f1, (double)f1), player);
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (!ItemTransformer.isLite) {
            if (event.world.isRemote && ItemPhysic.customPickup
                ...
                if (event.entityPlayer.worldObj.isRemote && entity != null) PacketHandler.sendPacketToServer(
                    new PickupPacket(event.entityPlayer.getLook(1.0F), event.entityPlayer.getPosition(1.0F)));
            }
        ...
        if (mc != null && mc.thePlayer != null && mc.inGameHasFocus) {
            if (ItemPhysic.customPickup) {
                ...
                if (mc.objectMouseOver != null)
                    if (mc.objectMouseOver.typeOfHit == MovingObjectType.BLOCK) distance = mc.thePlayer
