...
public class ServerPhysic {
    ...
    public static void update(EntityItem item) {
        ...
        if (stack != null && stack.getItem() != null) {
            if (stack.getItem()
                .onEntityItemUpdate(item)) return;
        }
        ...
        if (item.getEntityItem() == null) item.setDead();
            ...
            if (fluid == null) item.motionY -= 0.04D; // GRAVITY
            else {
                double density = (double) fluid.getDensity() / 1000D;
                double speed = -1 / density * 0.01;

                if (canItemSwim(stack, fluid)) speed = 0.05;
            ...
            item.noClip = func_145771_j(
                item,
                item.posX,
                (item.boundingBox.minY + item.boundingBox.maxY) / 2.0D,
                item.posZ);
            ...
            if (flag || item.ticksExisted % 20 == 0) {
                if (item.worldObj
                    ...
                    for (int zahl = 0; zahl < 100; zahl++) item.worldObj.spawnParticle(
                        "smoke",
                        item.posX,
                        item.posY,
                        item.posZ,
                ...
                if (item.onGround && canItemIgnite(stack)
                    && (item.worldObj
                        .getBlock(
                            MathHelper.floor_double(item.posX),
                            MathHelper.floor_double(item.posY),
            ...
            if (item.onGround) f = item.worldObj.getBlock(
                MathHelper.floor_double(item.posX),
    ...
    private static void searchForOtherItemsNearby(EntityItem item) {
        Iterator iterator = item.worldObj
            .getEntitiesWithinAABB(EntityItem.class, item.boundingBox.expand(0.5D, 0.0D, 0.5D))
            .iterator();
    ...
    public static void onCollideWithPlayer(EntityItem item, EntityPlayer par1EntityPlayer, boolean needsSneak) {
        ...
        if (!item.worldObj.isRemote) {
            ...
            if ((ItemPhysic.customPickup | item.delayBeforeCanPickup <= 0)
    ...
    public static boolean attackEntityFrom(EntityItem item, DamageSource par1DamageSource, float par2) {
        if (item.getEntityItem() != null) {
            if (isItemUndestroyable(item.getEntityItem()) || item.isEntityInvulnerable()) return false;
            else if (par1DamageSource.isExplosion() && !canItemExplode(item.getEntityItem())) return false;
            else if ((par1DamageSource == DamageSource.lava || par1DamageSource == DamageSource.onFire
                || par1DamageSource == DamageSource.inFire) && !canItemBurn(item.getEntityItem())) return false;
            else if (par1DamageSource == DamageSource.cactus && ItemPhysic.disableCactusDamage) return false;
            else if (ItemPhysic.isHBMLoaded && par1DamageSource == ModDamageSource.acid
                && !canItemDissolve(item.getEntityItem())) return false;
            else {
                if (par1DamageSource.isExplosion()) item.setDead();
    ...
    public static int getHealth(EntityItem item) {
        ...
        try {
            return EntityItem.class.getField(name)
                .getInt(item);
        } catch (Exception e) {
            try {
                return EntityItem.class.getField(name)
    ...
    public static void setHealth(EntityItem item, int health) {
        ...
        String name = ItemTransformer.patchT("health", obfuscated);
        try {
            EntityItem.class.getField(name)
                .setInt(item, health);
        } catch (Exception e) {
            try {
                EntityItem.class.getField(name)
                        ...
                        if (itemWithMetaFloat.liquids[i].equals(fluid.getUnlocalizedName())) {
                            if (!itemWithMetaFloat.ignoremeta) {
                                if (stack.getItem() == itemWithMetaFloat.item
                                    && stack.getItemDamage() == itemWithMetaFloat.metadata) {
                                    swim = true;
                                }
                            } else {
                                if (stack.getItem() == itemWithMetaFloat.item) {
                ...
                for (ItemsWithMetaRegistryBurn.ItemWithMetaBurn itemWithMetaBurn : ItemsWithMetaRegistryBurn.BurnItems) {
                    if (!itemWithMetaBurn.ignoremeta) {
                        if (stack.getItem() == itemWithMetaBurn.item
                            && stack.getItemDamage() == itemWithMetaBurn.metadata) {
                            burn = false;
                        }
                    } else {
                        if (stack.getItem() == itemWithMetaBurn.item) {
                ...
                for (ItemsWithMetaRegistryUndestroyable.ItemWithMetaUndestroyable itemWithMetaUndestroyable : ItemsWithMetaRegistryUndestroyable.UndestroyableItems) {
                    if (!itemWithMetaUndestroyable.ignoremeta) {
                        if (stack.getItem() == itemWithMetaUndestroyable.item
                            && stack.getItemDamage() == itemWithMetaUndestroyable.metadata) {
                            undestroyable = true;
                        }
                    } else {
                        if (stack.getItem() == itemWithMetaUndestroyable.item) {
                ...
                for (ItemsWithMetaRegistryIgniting.ItemWithMetaIgniting ItemWithMetaIgniting : ItemsWithMetaRegistryIgniting.IgnitingItems) {
                    if (!ItemWithMetaIgniting.ignoremetaItem) {
                        if (stack.getItem() == ItemWithMetaIgniting.item
                            && stack.getItemDamage() == ItemWithMetaIgniting.metadataItem) {
                            ignite = true;
                        }
                    } else {
                        if (stack.getItem() == ItemWithMetaIgniting.item) {
